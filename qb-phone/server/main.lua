...
local WebHook = "https://discord.com/api/webhooks/1001787848967602256/S-AGVHOSF14jDMfoqch4dASU72J8zw0q0v_J1DQUnBo9tJAZUWsUpi22-L9Xt0Uk8Jso"
local bannedCharacters = {'%','$',';'}

QBCore.Commands.Add("givesim", "Gives simcard with random number. (God Only)", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local info = {}
    info.tempNumber = "05" .. math.random(11111111,99999999)
    info.balance = 1

    Player.Functions.AddItem('simcard', 1, nil, info)
end, "god")

QBCore.Commands.Add("takeoffsim", "Take off your simcard.", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local oldPhoneData = Player.PlayerData.metadata["phonecachedata"]
    local citizenid = Player.PlayerData.citizenid

    if (oldPhoneData[citizenid].number ~= nil) and (oldPhoneData[citizenid].number ~= false) then
        local info = {}
        info.tempNumber = oldPhoneData[citizenid].number
        info.balance = oldPhoneData[citizenid].balance
        
        Player.Functions.AddItem('simcard', 1, nil, info)

        oldPhoneData[citizenid].number = false

        local playerInfo = Player.PlayerData.charinfo

        playerInfo.phone = "no-sim"

        Player.Functions.SetPlayerData('charinfo', playerInfo)
        Player.Functions.SetMetaData("phonecachedata", oldPhoneData)
        print(json.encode(oldPhoneData))
    else
        --notify you don't have sim
    end
end)

QBCore.Commands.Add("mynumber", "Gives phone number. (God Only)", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local cacheData = Player.PlayerData.metadata["phonecachedata"]
    local citizenid = Player.PlayerData.citizenid

    print(Player.PlayerData.charinfo.phone)
    print(json.encode(cacheData))
end, "god")

QBCore.Functions.CreateUseableItem("simcard", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    local playerInfo = Player.PlayerData.charinfo
    local firstNumber = Player.PlayerData.charinfo.phone
    local oldPhoneData = Player.PlayerData.metadata["phonecachedata"]
    local tempNumber = item.info["tempNumber"]
    local balance = item.info["balance"]

    local citizenid = Player.PlayerData.citizenid

    print("1 "  .. json.encode(oldPhoneData))
    if (type(oldPhoneData) == "table") then
        print("2 " .. json.encode(oldPhoneData))
        if oldPhoneData[citizenid].number ~= nil and oldPhoneData[citizenid].number ~= false then
            print("3 "  .. json.encode(oldPhoneData))
            local info = {}
        ...
        oldPhoneData[citizenid] = {
            number = tempNumber,
            balance = balance
        }
        print("4 " .. json.encode(oldPhoneData))
        ...
        print("5 " .. json.encode(oldPhoneData))
    ...
    playerInfo.phone = tempNumber

    Player.Functions.SetPlayerData('charinfo', playerInfo)

    Player.Functions.RemoveItem('simcard', 1, item.slot)
end)
...
QBCore.Functions.CreateCallback('qb-phone:server:GetCallState', function(source, cb, ContactData)
    ...
    local Player = QBCore.Functions.GetPlayer(source)
    local PhoneCacheData = Player.PlayerData.metadata["phonecachedata"]
    local citizenid = Player.PlayerData.citizenid

    if PhoneCacheData[citizenid].number ~= nil and PhoneCacheData[citizenid].number ~= false then
        if PhoneCacheData[citizenid].balance >= 1 then
            if Target ~= nil then
                if Calls[Target.PlayerData.citizenid] ~= nil then
                    if Calls[Target.PlayerData.citizenid].inCall then
                        cb(false, true, true, true)
                    else
                        PhoneCacheData[citizenid].balance = PhoneCacheData[citizenid].balance - 1
                        Player.Functions.SetMetaData("phonecachedata", PhoneCacheData)
...
QBCore.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    ...
    if Player ~= nil then
        ...
        if PhoneCacheData ~= nil then
            if PhoneCacheData[citizenid].number ~= nil or PhoneCacheData[citizenid].number ~= false then
                PhoneData.Cache = PhoneCacheData[citizenid]
            else
                PhoneData.Cache = false
            end
        else
            oldPhoneData = {}
